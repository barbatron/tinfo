...
import { get, put } from "./utils/LookupCache";
import { FetchDeparturesError } from "./utils/FetchDeparturesError";
...
function parseQueryFetchParams(
  ...
  const providerSpec = p(providerName);
  const s = stop_id ??
    (providerSpec.autoUpdate
      ? providerSpec.autoUpdateParams.stop_id
      : undefined);
  if (!s) throw new Error(`Unable to determine stop ID for ${providerName}`);
  return {
    stop_id: s,
    mot: !!mot ? String(mot.trim()) : undefined,
    dir: !!dir ? String(dir.trim()) : undefined,
    min_min: numberOr(min_min, 0),
    max_min: numberOr(max_min, 30),
    limit: numberOr(limit, 6),
  };
}
...
const updateDepartures = async (
  ...
  try {
    ...
    const cached = get(providerName, fp);
    const allDepartures = cached
      ? cached.departures
      : await prov.client.fetch(fp).then((deps) => put(providerName, fp, deps));
    const preferredMot = params?.mot;
...
const render = async (provider: Provider, params?: MinimumFetchParams) => {
  const prov = p(provider);
  const parms = params && Object.values(params).some((v) => !!v)
    ? params
    : undefined;
  console.log("fetch", provider, parms);
  const departures = await updateDepartures(provider, params);
...
const app = new Elysia()
  ...
  .get(
    ...
    async ({ query, params: { provider: providerStr }, set, headers }) => {
      const providerName = providerStr.toUpperCase() as Provider;
      log.info(
        "GET /:provider",
        { providerStr, query, provider: providerName },
        headers["user-agent"],
      );
      set.headers["content-type"] = "text/html";
      const fetchParams = parseQueryFetchParams(providerName, query);
      if (headers.accept === "application/json") {
        const departures = await updateDepartures(providerName, fetchParams);
        set.headers["content-type"] = "application/json";
        return decorateDepartures(departures);
      }
      const content = await render(providerName, fetchParams);
      return getIndex(
        { name: providerName, stopName: fetchParams.stop_id },
        content,
      );
    },
  )
  .get(
    ...
    async ({ query, params: { provider: providerStr }, headers, set }) => {
      const providerName = providerStr.toUpperCase() as Provider;
      if (!providers[providerName]) throw Error("Unknown provider");
      log.info(
